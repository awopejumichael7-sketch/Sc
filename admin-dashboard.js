...
import { generateBase32Secret, verifyTotp, buildOtpauthUri } from './totp.js';
...
function populateCourseSelects(){
  ...
  document.getElementById('driveCourse').innerHTML = `<option value="">General / All courses</option>` + opts;
}
...
/* ---------- Google Drive import (free: no OAuth, no API key — just a shared link) ---------- */
document.getElementById('driveForm').addEventListener('submit', async (e)=>{
  e.preventDefault();
  const url = document.getElementById('driveUrl').value.trim();
  if(!/drive\.google\.com/.test(url)){ toast('That does not look like a Google Drive link.', 'error'); return; }
  try{
    await addRecord('materials', {
      title: document.getElementById('driveTitle').value.trim(),
      type: document.getElementById('driveType').value,
      courseId: document.getElementById('driveCourse').value || null,
      url, source: 'google-drive', uploadedBy: session.uid
    });
    toast('Added from Google Drive.', 'success');
...
/* =========================================================================
   TWO-FACTOR AUTHENTICATION — free, offline TOTP (RFC 6238)
   Uses only the browser's built-in Web Crypto API (see totp.js) — no SMS
   gateway, no third-party auth service, no cost. Compatible with Google
   Authenticator, Authy, Microsoft Authenticator, or any standard TOTP app.
   ========================================================================= */
let pendingSecret = null;
document.getElementById('generate2faBtn').addEventListener('click', ()=>{
  pendingSecret = generateBase32Secret();
  const uri = buildOtpauthUri(pendingSecret, session.email || session.uid);
  document.getElementById('twoFaResult').classList.remove('hidden');
  document.getElementById('twoFaSecret').textContent = pendingSecret;
  document.getElementById('twoFaQr').innerHTML = '';
  new QRCode(document.getElementById('twoFaQr'), { text: uri, width:164, height:164 });
});
document.getElementById('verify2faBtn').addEventListener('click', async ()=>{
  const entered = document.getElementById('twoFaVerifyInput').value.trim();
  if(!pendingSecret || entered.length !== 6){ toast('Enter the 6-digit code from your authenticator app.', 'error'); return; }
  const statusBox = document.getElementById('twoFaStatus');
  if(await verifyTotp(pendingSecret, entered)){
    try{
      await updateRecord('admins', session.uid, { totpSecret: pendingSecret, totpEnabled: true });
      statusBox.classList.add('show');
      statusBox.textContent = 'Two-factor authentication enabled for your account.';
      toast('2FA enabled.', 'success');
    }catch(err){ toast('Could not save 2FA settings: ' + err.message, 'error'); }
  }else{
    statusBox.className = 'alert alert-error show';
    statusBox.textContent = 'That code did not match — check the time on your device and try again.';
  }
});
