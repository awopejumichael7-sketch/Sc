...
import { getAllRecords, addRecord, setRecord, watchSession, requestNotificationPermission, onForegroundMessage } from './firebase-config.js';
...
async function loadEverything(){
  // Fetch every collection independently (instead of one long await chain) so
  // that a single failed/denied read can never stop the rest of the page from
  // loading and rendering. Each fetch reports its own error via toast instead
  // of silently killing everything that comes after it.
  const [allCoursesR, materialsR, examQuestionsR, announcementsR, gradesR] = await Promise.allSettled([
    getAllRecords('courses'),
    getAllRecords('materials'),
    getAllRecords('examQuestions'),
    getAllRecords('announcements'),
    getAllRecords('grades')
  ]);
  ...
  materials = (materialsR.status === 'fulfilled' ? materialsR.value : [])
    .filter(m => !m.courseId || (session.courseIds||[]).includes(m.courseId));
  examQuestions = (examQuestionsR.status === 'fulfilled' ? examQuestionsR.value : [])
    .filter(q => (session.courseIds||[]).includes(q.courseId));
  announcements = announcementsR.status === 'fulfilled' ? announcementsR.value : [];
  grades = gradesR.status === 'fulfilled' ? gradesR.value : [];

  [allCoursesR, materialsR, examQuestionsR, announcementsR, gradesR].forEach(r=>{
    if(r.status === 'rejected') console.error('Dashboard data load failed:', r.reason);
  });
  if([allCoursesR, materialsR, examQuestionsR, announcementsR, gradesR].some(r=> r.status==='rejected')){
...
/* =========================================================================
   LEADERBOARD — uses a dedicated, minimal-field 'leaderboard' collection
   (name + class level + XP only) so students can see rankings without
   Firestore rules having to expose classmates' full profile records.
   ========================================================================= */
async function renderLeaderboard(){
  const myXp = (myCourses.length * 120) + (grades.length * 40);
  try{
    await setRecord('leaderboard', session.uid, { name: session.fullName || 'Student', classLevel: session.classLevel || '—', xp: myXp });
  }catch(err){ /* non-fatal — leaderboard is a nice-to-have, not core functionality */ }

  const all = await getAllRecords('leaderboard');
  const sameLevel = all.filter(r => r.classLevel === session.classLevel).sort((a,b)=> (b.xp||0) - (a.xp||0));
  const body = document.querySelector('#leaderboardTable tbody');
  if(!sameLevel.length){ body.innerHTML = `<tr><td colspan="3" class="text-center text-muted" style="padding:30px;">No rankings yet.</td></tr>`; return; }
  body.innerHTML = sameLevel.map((r,i)=>`
    <tr style="${r.id===session.uid ? 'background:var(--surface-2);font-weight:700;' : ''}">
      <td>${i===0?'🥇':i===1?'🥈':i===2?'🥉':`#${i+1}`}</td>
      <td>${escapeHtml(r.name)}${r.id===session.uid ? ' <span class="badge badge-gold">You</span>' : ''}</td>
      <td>${r.xp||0} XP</td>
    </tr>`).join('');
}

/* =========================================================================
   DIGITAL ID CARD — QR code encodes the student's ID for fast front-desk
   or exam-hall verification. Generated entirely client-side (QRCode.js,
   free/MIT), no server round-trip.
   ========================================================================= */
function renderDigitalId(){
  document.getElementById('idAvatar').textContent = (session.fullName||'S').charAt(0).toUpperCase();
  document.getElementById('idName').textContent = session.fullName || '—';
  document.getElementById('idClass').textContent = session.classLevel || '—';
  document.getElementById('idCode').textContent = session.id || '—';
  const qrWrap = document.getElementById('idQr');
  qrWrap.innerHTML = '';
  new QRCode(qrWrap, { text: session.id || session.uid, width: 120, height: 120 });
}
document.getElementById('downloadIdBtn').addEventListener('click', ()=>{
  const canvas = document.querySelector('#idQr canvas');
  if(!canvas){ toast('ID card not ready yet.', 'error'); return; }
  const link = document.createElement('a');
  link.download = `${session.id || 'student'}-id-card.png`;
