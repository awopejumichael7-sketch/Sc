...
export async function getAllRecords(col, ...constraints){
  ...
  // docId is always the REAL Firestore document ID (e.g. the student/teacher's
  // Auth uid). It's listed first and never collides, since none of our data
  // ever stores a field literally called "docId". `id` is kept as-is for
  // backward compatibility: for most collections it ends up equal to the
  // Firestore doc ID, but for students/teachers the stored data.id (the
  // human-readable Student/Teacher ID, e.g. SC-STU-2026-0001) intentionally
  // overrides it for display — docId is what callers must use for
  // updateRecord/deleteRecord, never id.
  return snap.docs.map(d=>({ docId:d.id, id:d.id, ...d.data() }));
}
export function listenToCollection(col, cb, ...constraints){
  const q = constraints.length ? query(collection(db, col), ...constraints) : collection(db, col);
  return onSnapshot(q, snap=> cb(snap.docs.map(d=>({ docId:d.id, id:d.id, ...d.data() }))));
}
